...
| **Runner management** | System Wine or Proton, auto-detected from the same compatibility-tools directories (and precedence order) Steam itself scans | Own runner management incl. its own "Bottles Runtime", many Wine forks selectable | Proton Manager to download/remove runners (GE-Proton, Proton-EM), sourced from Steam's `compatibilitytools.d` |
